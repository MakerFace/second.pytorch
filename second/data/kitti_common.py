...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if with_imageshape:
            ...
            image_info['image_shape'] = (1920, 1080)
        if label_info:
...
def get_label_anno(label_path):
    ...
    num_objects = len([x[1] for x in content if x[1] != 'DontCare'])
    annotations['name'] = np.array([x[1] for x in content])
    num_gt = len(annotations['name'])
    annotations['truncated'] = np.array([float(x[2]) for x in content])
    annotations['occluded'] = np.array([int(x[3]) for x in content])
    annotations['alpha'] = np.array([float(x[-1]) for x in content])
    annotations['bbox'] = np.array(
        [[float(0) for info in x[4:8]] for x in content]).reshape(-1, 4)
    # dimensions will convert hwl format to standard lhw(camera) format.
    annotations['dimensions'] = np.array(
        [[float(info) for info in x[4:7]] for x in content]).reshape(
            -1, 3)[:, [2, 0, 1]]
    annotations['location'] = np.array(
        [[float(info) for info in x[7:10]] for x in content]).reshape(-1, 3)
    annotations['rotation_y'] = np.array(
        [float(0) for x in content]).reshape(-1)
    if len(content) != 0 and len(content[0]) == 16:  # have score
        annotations['score'] = np.array([float(0) for x in content])
    else:
